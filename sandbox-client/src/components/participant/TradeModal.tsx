...
export const TradeModal: React.FC<TradeModalProps> = ({ stock, mode, onClose }) => {
  ...
  const handleTrade = async (e: React.FormEvent) => {
    ...
    const res =
      mode === "BUY"
        ? await executeBuy(stock.id, quantity)
        : await executeSell(stock.id, quantity);
      ...
      <DialogContent className="sm:max-w-[420px]">
        <DialogHeader className="border-b border-border pb-4">
          <div className="flex items-center gap-2 pr-8">
            ...
            <DialogTitle className="text-base">{stock.symbol}</DialogTitle>
            <span className="text-xs text-muted-foreground">{stock.name}</span>
          </div>
          <DialogDescription className="sr-only">
            {mode} order for {stock.symbol}
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleTrade} className="flex flex-col gap-4 pt-1">
          ...
          <div className="flex flex-col gap-1.5 rounded-md border border-border bg-muted/60 p-3 text-sm">
            <div className="flex justify-between text-muted-foreground">
              ...
              <span className="tabular-nums">{formatINR(stock.currentPrice)}</span>
            </div>
            <div className="flex justify-between text-muted-foreground">
