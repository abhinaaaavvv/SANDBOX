...
export const LoginForm: React.FC<LoginFormProps> = ({ mode }) => {
  ...
  const openDemo = (demoMode: AuthRole) => {
    const result = signInDemo(demoMode);
    router.replace(result.role === "admin" ? "/admin" : "/participant");
  };
      ...
      <main className="flex flex-1 items-center justify-center px-6">
        <div className="w-full max-w-sm">
          ...
          <div className="space-y-2 text-center text-xs text-muted-foreground">
            ...
            <p className="font-medium text-foreground">Local demo only:</p>
            <p>{DEMO_ACCOUNTS.admin.email} / {DEMO_ACCOUNTS.admin.password}</p>
            <p>{DEMO_ACCOUNTS.participant.email} / {DEMO_ACCOUNTS.participant.password}</p>
            <div className="mt-3 flex gap-2">
              <Button type="button" variant="secondary" className="flex-1" onClick={() => openDemo("participant")}>
                Open Participant Demo
              </Button>
              <Button type="button" variant="secondary" className="flex-1" onClick={() => openDemo("admin")}>
                Open Admin Demo
              </Button>
            </div>
          </div>
